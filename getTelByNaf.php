...
$aTabStockNaf = $oConnectPG->getStockByNaf($naf);

echo "NB ETAB = " . count($aTabStockNaf) . "\n";


for ($l = 0; $l < count($aTabStockNaf); $l++) {
    ...
    $adresse = $oConnectPG->formatAdressePourBanoTableau($aTabStockNaf[$l]);


    $nom_old = $aTabStockNaf[$l]['denomination_geoscar'];
    $siret = $aTabStockNaf[$l]['siret'];
    ...
    if (strlen($adresse) > 0 && strlen($nom) > 0) {
        ...
        if ($resultat) {
            ...
            echo "http://192.168.1.52:8080/REST_API_PJGeosiren/geosiren.com/apipj/search?siret=" . $siret . "&nom=" . $nom . "&adresse=" . $adresse . " \n";
            if ($resultat->m_head->m_status == "200") {
                ...
                $oConnectPG->updateTelGeosirene($aTabStockNaf[$l]['gid'], $sNumTel, $iScore);
                //$oConnectPG->updateTelStock($aTabStockNaf[$l]['siret'], $sNumTel, $iScore);
                $oConnectPG->insertTablePJ($aTabStockNaf[$l]['siret'], $sNumTel, $iScore);
                //$oConnectPG->ajoutFichierPagesJaunes($aTabStockNaf[$l]['siret'], $sNumTel, $iScore);
            } else {

                $gid = $aTabStockNaf[$l]['gid'];
